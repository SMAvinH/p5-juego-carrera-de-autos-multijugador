class Game {
  ...
  async start() {
    ...
    // Cuando el usuario ingresa exitosamente.
    if (gameState === 0) {
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      ...
      //índice del agrreglo / index of the array
      var index = 0;

      for (var plr in allPlayers) {
        //Agregar 1 al índice por cada bucle o loop 
        index = index + 1;
        ...
        if (index === player.index) {
          ...
          // Cambiar la posicón de la cámara en dirección y 
  ...
  showLeaderboard() {
    ...
    if (
      ...
      // &emsp;   Ésta "etiqueta" se usa para desplegar cuatro espacios.
      leader1 =
        players[0].rank +
        "&emsp;" +
        players[0].name +
  ...
  handleFuel(index) {
    ...
    cars[index - 1].overlap(fuels, function(collector, collected) {
      player.fuel = 185;
      //"collected" es el "sprite" en el grupo "collectibles" que desencadena
      //el evento.
      collected.remove();
    });

    // Reducción del combustible del jugador.
  ...
  showRank() {
    swal({
      title: `¡Impresionante!${"\n"}Posición${"\n"}${player.rank}`,
      text: "Alcanzaste la meta con éxito",
      imageUrl:
